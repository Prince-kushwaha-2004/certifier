...
export default function CreateCertificate() {
    let { showimage, boxstart, drawbox, boxstop, addField, changetitle, handleDelete, handleSubmit, clearform, changeAlign, changeFont, changeColor, isSelected, data, downloadLink, color } = useCertificate()
            ...
            <form action="" className='w-full  flex flex-col md:flex-row h-[55rem]' onSubmit={handleSubmit}>
                <div className='w-full md:w-1/2 lg:w-2/3 flex flex-col justify-center items-center bg-white border-r border-gray-300 overflow-hidden'>
                    ...
                    </label>
                    {
                        data.map((value, index) => {
                            return (
                                <div key={index} className='border flex items-center justify-center rounded' style={value.boxstyle ? value.boxstyle : { display: "none" }}><h1>{value.key}</h1></div>
                            )
                        })
                    }
                ...
                <div className='w-full md:w-1/2 lg:w-1/3 p-4 overflow-auto flex-1'>
                    ...
                    <div className='flex flex-row-reverse'>
                        ...
                        <select name="cars" id="cars" className='bg-slate-100 focus:outline-none border my-auto px-3 py-2 rounded-lg border-blue-300' onChange={changeFont}>
                            <option value="Fira Sans Condensed" style={{ fontFamily: "Fira Sans Condensed" }}>Fira Sans Condensed</option>
                            <option value="Playwrite AU SA" style={{ fontFamily: "Playwrite AU SA" }}>Playwrite AU SA</option>
                            <option value="Merriweather" style={{ fontFamily: "Merriweather" }}>Merriweather</option>
                            <option value="Dancing Script" style={{ fontFamily: "Dancing Script" }}>Dancing Script</option>
                            <option value="Parisienne" style={{ fontFamily: "Parisienne" }}>Parisienne</option>
                            <option value="Faustina" style={{ fontFamily: "Faustina" }}>Faustina</option>
                            <option value="Carter One" style={{ fontFamily: "Carter One" }}>Carter One</option>
                        </select>
                        <input type="color" id="color" value={color} className='w-10 h-10 my-auto mx-2 cursor-pointer' onChange={changeColor} />
                        <label htmlFor="color" className='w-10 h-10 my-auto text-4xl text-gray-600 cursor-pointer'><IoIosColorPalette /></label>
